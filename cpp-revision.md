...
- [Phase 1 — Core C++ Language Features](#phase-1)
...
- [Phase 2 — Resource Management & RAII](#phase-2)
...
### What the Compiler Actually Does — Preprocessing → Compilation → Assembly → Linking
...
### Linking — Static vs Dynamic, Symbol Resolution, Undefined References
...
### Static Libraries (.a) vs Shared Libraries (.so / .dll / .dylib)
...
### Implementation-Defined vs Unspecified vs Undefined Behavior
...
<a name="phase-1"></a>
## Phase 1 — Core C++ Language Features (Intermediate → Advanced)

<a name="p1-1"></a>
### 1.1 — Type System Deep Dive

#### Fundamental Types & Fixed-Width Integers (`<cstdint>`)
...
#### Scoped Enums: `enum class` vs Plain `enum`
...
#### `reinterpret_cast` and Aliasing Rules
...
#### Ownership Semantics & RAII Philosophy
...
<a name="p1-3"></a>
### 1.3 — Classes & Object-Oriented Programming In Depth
...
#### Initialization Kinds: Direct, Copy, List, Aggregate
...
#### Copy Constructor & Copy Assignment — Deep vs Shallow Copy
...
#### Aggregate Types & Aggregate Initialization
...
#### `virtual` Functions, vtable, vptr — Dynamic Dispatch
...
#### Dependent Names — `typename` and `template` Disambiguation Keywords
...
#### `std::pair` and `std::tuple`, Structured Bindings
...
#### `std::string`, SSO, `std::string_view`
...
#### `begin()`/`end()` Family & Range-Based `for`
...
#### Algorithms: `sort`, `find`, `transform`, `accumulate`, and friends
...
#### `std::exception` and Standard Exception Types
...
#### `std::runtime_error` vs `std::logic_error`
...
#### RAII (Resource Acquisition Is Initialization) — The Core Philosophy
...
#### RAII with File Handles, Locks, Memory
...
#### Lock Guards: `lock_guard`, `unique_lock`, `shared_lock`, `scoped_lock`
...
#### Guard Pattern — Action-on-Destruction Object
...
#### Scope Exit — Running Code When Leaving a Scope
...
#### Why Raw `new`/`delete` Is Almost Always Wrong in Modern C++
...
#### `std::scoped_lock` — Multi-Mutex, Deadlock-Free Locking
...
#### `std::atomic_flag` — Simplest Atomic, Spinlock Implementation
...
### 3.5 — Concurrency Patterns & Pitfalls
...
#### Reader-Writer Locks — Writer Starvation
...
#### Buffer Management — Raw `char` Arrays
...
#### Formatting — `fmt` library, `std::format` (C++20)
...
<a name="phase-6"></a>
## Phase 6 — Build Systems & Toolchain
...
#### Compiling, Linking, Include/Library Paths, Warnings, Optimization, Debug Info, Standard Selection
...
#### Variables, Automatic Variables, Pattern Rules, Phony Targets
...
#### What is CMake & Core `CMakeLists.txt` Commands
...
#### Build Types, Flags, Out-of-Source Builds, Binary/Source Dirs
...
#### Generator Expressions, `option()`, Custom Commands/Targets, CTest Integration
...
#### What is GoogleTest, `TEST()`, `TEST_F()`
...
#### Mock Classes, `MOCK_METHOD`, `EXPECT_CALL`
...
#### Breakpoints, Stepping, Inspecting Variables, Backtrace, Watchpoints, Conditional Breakpoints
...
#### AddressSanitizer (ASan), ThreadSanitizer (TSan), UndefinedBehaviorSanitizer (UBSan), MemorySanitizer (MSan), LeakSanitizer (LSan)
...
#### Benchmarking — Google Benchmark, `std::chrono`
...
#### `std::cerr` vs `std::cout` — Buffering
...
#### Logging Libraries — spdlog, log4cxx
...
#### `.clang-format` File & `BasedOnStyle: Google`
...
#### Compiler Warnings as Errors — `-Werror`, `-Wall -Wextra`
...
#### Header File Organization & Forward Declarations
...
<a name="phase-10"></a>
## Phase 10 — Systems Programming Concepts

### 10.1 — Operating System Fundamentals
...
#### File Systems — Inodes, Directory Structure, Block Devices
...
#### HDD vs SSD — Seek Time, Latency, Sequential vs Random I/O
...
#### `fsync` — Flushing to Durable Storage
...
<a name="phase-13"></a>
## Phase 13 — Development Environment & Workflow
...
#### Vim/Neovim for Server-Based Development
...
#### What is Docker, Dockerfile, `.dockerignore`
...
#### Why Projects Provide Docker Files
...
#### Singleton Pattern (and Why It's Often Bad)
